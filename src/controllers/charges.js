...
try {
    const overdue = await knex.select('cobranca.*', 'cliente.nome')
                                            .from('cobranca')
    ...
    return res.status(200).json({cobrancas_vencidas:overdue, Total_Vencido:totalDue});
} catch (error) {
...
const expectedCharges = async (req,res) => {
    try {
    const expected = await await knex.select('cobranca.*', 'cliente.nome')
                                            .from('cobranca')
                                            .join('cliente', 'cobranca.cliente_id', 'cliente.cliente_id')
...
const paidCharges = async (req,res)=>{
        try {
    const paid = await knex.select('cobranca.*', 'cliente.nome')
                                            .from('cobranca')
                                            .join('cliente', 'cobranca.cliente_id', 'cliente.cliente_id')
